...
/**
 * Provider InterventionWorkItemProvider.
 *
 * @category Provider
 *
 * @version 1.0.0
 ...
 *
 * @implements ProviderInterface<InterventionWorkItemOutput>
 */
final readonly class InterventionWorkItemProvider implements ProviderInterface
{
   ...
   * Constructor.
   *
   * Initializes a new instance of the InterventionWorkItemProvider class.
   *
   * @since 1.0.0
   *
   ...
   *
   * @return InterventionWorkItemOutput|TraversablePaginator the provide result
   */
  public function provide(Operation $operation, array $uriVariables = [], array $context = []): InterventionWorkItemOutput|TraversablePaginator
  {
    $user = $this->user();
    ...
    if (is_string($id)) {
      try {
        ...
        $result = $this->queryBus->ask(new GetInterventionWorkflowQuery($user->getId(), 'work_item', $id));
