...
use Mission\Application\UseCase\Command\Publication\RequestPublication\{RequestPublicationCommand, RequestPublicationResult};
  ...
  public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): PublicationOutput
  {
    ...
    try {
      ...
      $result = $this->commandBus->dispatch(new RequestPublicationCommand(
        $user->getId(),
        ResourceIriParser::id($data->mission, 'missions'),
        $data->missionRevision,
      ));
    } catch (MessengerRuntimeException $exception) {
      throw $this->mapException($exception);
    } catch (MissionAccessDeniedException|MissionBlockedException|MissionConflictException|MissionNotFoundException $exception) {
      throw $this->mapException($exception);
    }
  ...
  private function mapException(Throwable $exception): Throwable
  {
    ...
    do {
      $mapped = match (true) {
        ...
        $current instanceof MissionBlockedException => new UnprocessableEntityHttpException($current->getMessage(), $exception),
        default => null,
      };
