...
/**
 * Provider MissionChangeProvider.
 *
 * @category Provider
 *
 * @version 1.0.0
 ...
 *
 * @implements ProviderInterface<MissionChangeOutput>
 */
final readonly class MissionChangeProvider implements ProviderInterface
{
   ...
   * Constructor.
   *
   * Initializes a new instance of the MissionChangeProvider class.
   *
   * @since 1.0.0
   *
   ...
   *
   * @return MissionChangeOutput|TraversablePaginator the provide result
   */
  public function provide(Operation $operation, array $uriVariables = [], array $context = []): MissionChangeOutput|TraversablePaginator
  {
    $user = $this->user();
    ...
    if (is_string($id)) {
      try {
        ...
        $result = $this->queryBus->ask(new GetMissionWorkflowQuery($user->getId(), 'change', $id));
