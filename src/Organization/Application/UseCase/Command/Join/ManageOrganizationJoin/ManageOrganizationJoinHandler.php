...
final readonly class ManageOrganizationJoinHandler implements CommandHandler
{
  ...
  public function __invoke(ManageOrganizationJoinCommand $command): ManageOrganizationJoinResult
  {
    if ('accept_invitation' === $command->operation) {
      return $this->acceptInvitation($command);
    }
    $orgId = $this->organizationId($command);
    $this->authorizeOperation($command, $orgId);
    $memberAdded = null;
    $changedRequest = null;
    $changed = false;
    ...
    $result = $this->transactionManager->transactional(function () use ($command, $orgId, &$memberAdded, &$changedRequest, &$changed, &$invalidatedEmail): ManageOrganizationJoinResult {
      return $this->executeLockedOperation($command, $orgId, $memberAdded, $changedRequest, $changed, $invalidatedEmail);
    });
    $this->afterCommit($command, $orgId, $memberAdded, $changedRequest, $changed, $invalidatedEmail);
  ...
  private function decideRequest(ManageOrganizationJoinCommand $command, string $orgId, DateTimeImmutable $now, ?AddOrganizationMemberResult &$memberAdded, ?OrganizationJoinRequest &$changedRequest, bool &$changed, bool &$invalidatedEmail): ManageOrganizationJoinResult
  {
    ...
    if ('approve' === $command->operation) {
      ...
      if ($this->approvalEmailInvalid($request, $email)) {
  ...
  private function joinOrRequest(ManageOrganizationJoinCommand $command, string $orgId, DateTimeImmutable $now, ?AddOrganizationMemberResult &$memberAdded, ?OrganizationJoinRequest &$changedRequest, bool &$changed): ManageOrganizationJoinResult
  {
    ...
    $pending = $this->findPendingRequest($command->userId, $orgId, $proof, $now);
    if ('join' === $command->operation) {
      return $this->join($command, $orgId, $policy, $member, $pending, $memberAdded, $changed);
    }
  ...
  {
    $pending = null;
    foreach ($this->joins->requests($userId, $orgId) as $request) {
      if ('pending' === $request->status && $this->staleRequest($request, $proof, $now)) {
  ...
  /**
   * @param-out AddOrganizationMemberResult $memberAdded
   */
  private function join(ManageOrganizationJoinCommand $command, string $orgId, OrganizationAccessPolicy $policy, ?OrganizationMember $member, ?OrganizationJoinRequest $pending, ?AddOrganizationMemberResult &$memberAdded, bool &$changed): ManageOrganizationJoinResult
  {
    if (null !== $pending) {
