  ...
  operations: [
    new Get(
      ...
      openapi: new Operation(
        ...
        description: 'Returns organization dashboard analytics. `overview`, `alerts`, and non-`period*` health KPIs are current snapshots evaluated at `generatedAt`; `trendMetrics`, `comparison`, and `period*` health KPIs are scoped to the requested period. `overview` is normalized for frontend consumption and exposes only stable keys, raw values, `summary`, and `breakdowns`. `alerts` expose machine-readable `code`/`severity`/`count`, `health` exposes an iterable `metrics` list, `comparison` exposes iterable `metrics` and `health.metrics` lists, and `trendMetrics` provides a directly iterable chart catalog without presentation labels. Analytics filters are section-scoped: `facilityType` affects facilities, `equipmentType` and `equipmentStatus` affect equipment, `inspection*` filters affect inspection metrics and trends, and `nonConformity*` filters affect non-conformity metrics, alerts, and trends. By default, the endpoint returns the latest 30-day window in UTC with daily buckets and compares it with the previous period. The maximum supported period is 366 days. Datetime bounds must be ISO 8601 strings with an explicit timezone offset, including optional microseconds. Access requires `organization.dashboard.read` plus the underlying read permissions for members, roles, facilities, equipment, and inspections. For chart-specific inspection or non-conformity periods or granularities, use the dedicated `/dashboard/trends/*` endpoints, which require `organization.inspection.read`.',
        parameters: [
          new Parameter(
            name: 'from',
            in: 'query',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Whether to include previous-period comparison metrics. Defaults to true.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Trend aggregation granularity. Allowed values: day, week, month, auto. Defaults to day. Use auto to adapt buckets to the requested period length.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'IANA timezone used for daily bucket boundaries and rendered period values. Defaults to UTC when omitted and not implied by the request bounds. Required when the requested period spans DST, mixes offsets, or uses non-UTC numeric offsets.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Optional inspection status filter applied to inspection KPIs, period health, and inspection trends.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Optional inspection result filter applied to inspection KPIs, period health, and inspection trends.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Optional inspector type filter applied to inspection KPIs, period health, and inspection trends.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Optional non-conformity status filter applied to non-conformity KPIs, alerts, period health, and non-conformity trends.',
          ...
          new Parameter(
            ...
            in: 'query',
            required: false,
            description: 'Optional non-conformity severity filter applied to non-conformity KPIs, alerts, period health, and non-conformity trends.',
