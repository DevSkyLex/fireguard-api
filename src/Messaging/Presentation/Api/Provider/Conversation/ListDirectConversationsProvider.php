...
/**
 * Provider ListDirectConversationsProvider.
 *
 * @category Provider
 *
 * @version 1.0.0
 ...
 *
 * @implements ProviderInterface<\Messaging\Presentation\Api\Dto\Output\ConversationOutput>
 */
final readonly class ListDirectConversationsProvider implements ProviderInterface
{
  ...
  public function provide(Operation $operation, array $uriVariables = [], array $context = []): object
  {
    ...
    try {
      ...
      $result = $this->queryBus->ask(new ListDirectConversationsQuery(
        userId: $user->getId(),
    ...
    $items = array_map(
      fn ($view) => $this->mapper->fromView(
        $view,
        null,
        $result->unreadCounts[$view->id] ?? 0,
        in_array($view->id, $result->favoriteConversationIds, true),
        $result->counterpartMemberIds[$view->id] ?? null,
      ),
      $result->page->items,
    );
