...
use Messaging\Application\UseCase\Command\Message\DeleteMessage\DeleteMessageCommand;
...
/**
 * Processor DeleteMessageProcessor.
 *
 * Handles `DELETE /api/messages/{id}` (tombstone; `204 No Content`).
 *
 * @category Processor
 *
 * @version 1.0.0
...
final readonly class DeleteMessageProcessor implements ProcessorInterface
{
  ...
  public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): void
  {
    ...
    try {
      $this->commandBus->dispatch(new DeleteMessageCommand(
        userId: $user->getId(),
        messageId: $id,
      ));
