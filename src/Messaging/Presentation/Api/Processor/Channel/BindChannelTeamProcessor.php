...
use Messaging\Application\UseCase\Command\Channel\BindChannelTeam\{BindChannelTeamCommand, BindChannelTeamResult};
...
/**
 * Processor BindChannelTeamProcessor.
 *
 * Handles `PATCH /api/channels/{id}/team` (bind when `teamId` is set,
 * unbind when it is `null`).
 *
 * @category Processor
 *
 * @version 1.0.0
 ...
 *
 * @implements ProcessorInterface<BindChannelTeamInput, ChannelOutput>
 */
final readonly class BindChannelTeamProcessor implements ProcessorInterface
{
  ...
  public function process(mixed $data, Operation $operation, array $uriVariables = [], array $context = []): ChannelOutput
  {
    ...
    if (!$data instanceof BindChannelTeamInput) {
      throw new BadRequestHttpException('Invalid request body.');
    }

    try {
      /** @var BindChannelTeamResult $result */
      $result = $this->commandBus->dispatch(new BindChannelTeamCommand(
        userId: $user->getId(),
        conversationId: $id,
        teamId: $data->teamId,
      ));
