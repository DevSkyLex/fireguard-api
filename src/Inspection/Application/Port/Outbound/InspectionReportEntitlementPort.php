...
/**
 * Port InspectionReportEntitlementPort.
 *
 * Resolves whether an organization's current plan tier entitles it to
 * export the Inspection module's PDF documents — the single inspection
 * report and the non-conformities report (reserved to `pro`/`max`, the SAME
 * gate as the Compliance safety register: a deliberate product decision
 * aligning every PDF document export on one entitlement). Implemented by
 * the same Organization-module adapter as
 ...
 */
interface InspectionReportEntitlementPort
{
  // #region Methods
  /**
   ...
   *
   * @return bool whether the organization's plan entitles it to export the Inspection PDF reports
   */
