...
# the floor-plan images where the API will never look: the download endpoint
...
seed-fixtures-docker:
	docker compose exec -T app php -d memory_limit=$(PHP_MEMORY_LIMIT) $(CONSOLE_BIN) app:fixtures:load --no-interaction
