...
The complete-source 90% gate on `main` merges all five suites into one report.
If CI exposes a historical coverage gap and blocks the SonarQube scan, promote
the normal test backlog needed to meet the threshold; do not weaken the gate or
mark deployment ready to bypass that result.
