...
use Messaging\Application\UseCase\Command\Message\PinMessage\{PinMessageCommand, PinMessageResult};
...
use Messaging\Presentation\Api\Processor\Message\PinMessageProcessor;
...
/**
 * Test PinMessageProcessorTest.
 *
 * @category Processor Tests
 *
 * @author Valentin FORTIN <contact@valentin-fortin.pro>
 */
#[CoversClass(PinMessageProcessor::class)]
final class PinMessageProcessorTest extends TestCase
{
  ...
  #[Test]
  public function testProcessDispatchesThePinCommandAndReturnsTheOutput(): void
  {
    /** @var CommandBusPort&MockObject $commandBus */
    $commandBus = $this->createMock(CommandBusPort::class);
    $commandBus->expects(self::once())
      ->method('dispatch')
      ->with(self::callback(static fn (PinMessageCommand $command): bool => self::USER_ID === $command->userId
        && self::MESSAGE_ID === $command->messageId))
      ->willReturn(new PinMessageResult($this->view(), 'pinner-1'));

    $processor = new PinMessageProcessor($commandBus, $this->outputFactory(), $this->securityWithUser());

    $output = $processor->process(null, new Post(), ['id' => self::MESSAGE_ID]);

    self::assertInstanceOf(MessageOutput::class, $output);
    self::assertNotNull($output->pinnedAt);
  }

  #[Test]
  public function testProcessThrowsWhenIdIsMissing(): void
  {
    $processor = new PinMessageProcessor($this->createStub(CommandBusPort::class), $this->outputFactory(), $this->securityWithUser());
  ...
  public function testProcessThrowsWhenNotAuthenticated(): void
  {
    ...
    $processor = new PinMessageProcessor($this->createStub(CommandBusPort::class), $this->outputFactory(), $security);
