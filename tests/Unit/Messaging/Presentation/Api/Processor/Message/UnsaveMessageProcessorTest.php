...
use Messaging\Application\UseCase\Command\Message\UnsaveMessage\{UnsaveMessageCommand, UnsaveMessageResult};
use Messaging\Domain\Exception\MessagingAccessDeniedException;
use Messaging\Presentation\Api\Processor\Message\UnsaveMessageProcessor;
...
/**
 * Test UnsaveMessageProcessorTest.
 *
 * @category Processor Tests
 *
 * @author Valentin FORTIN <contact@valentin-fortin.pro>
 */
#[CoversClass(UnsaveMessageProcessor::class)]
final class UnsaveMessageProcessorTest extends TestCase
{
  ...
  #[Test]
  public function testProcessDispatchesTheUnsaveCommand(): void
  {
    /** @var CommandBusPort&MockObject $commandBus */
    $commandBus = $this->createMock(CommandBusPort::class);
    $commandBus->expects(self::once())
      ->method('dispatch')
      ->with(self::callback(static fn (UnsaveMessageCommand $command): bool => self::USER_ID === $command->userId
        && self::MESSAGE_ID === $command->messageId))
      ->willReturn(new UnsaveMessageResult($this->view()));

    $processor = new UnsaveMessageProcessor($commandBus, $this->securityWithUser());
  ...
  {
    $processor = new UnsaveMessageProcessor($this->createStub(CommandBusPort::class), $this->securityWithUser());
  ...
  public function testProcessThrowsWhenNotAuthenticated(): void
  {
    ...
    $processor = new UnsaveMessageProcessor($this->createStub(CommandBusPort::class), $security);
