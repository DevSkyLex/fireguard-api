...
namespace Tests\Unit\Messaging\Application\UseCase\Command\Message\AddReaction;
...
use Messaging\Application\Port\Outbound\{MessagingConversationRepositoryPort, MessagingMemberDirectoryPort, MessagingMessageRepositoryPort, MessagingParticipantRepositoryPort, MessagingReactionRepositoryPort, MessagingRealtimePublisherPort, MessagingSubjectResolverPort};
use Messaging\Application\Service\{MessagingAccessPolicy, MessagingSubjectResolverRegistry};
use Messaging\Application\UseCase\Command\Message\AddReaction\{AddReactionCommand, AddReactionHandler};
...
/**
 * Test AddReactionHandlerTest.
 *
 * @category Handler Tests
 *
 * @author Valentin FORTIN <contact@valentin-fortin.pro>
 */
#[CoversClass(AddReactionHandler::class)]
final class AddReactionHandlerTest extends TestCase
{
  ...
  #[Test]
  public function testInvokeAddsTheReactionAndPublishesRealtime(): void
  {
    ...
    $reactions->expects(self::once())
      ->method('add')
      ->with(self::MESSAGE_ID, self::ORG_ID, self::REACTING_MEMBER_ID, "\u{1F44D}");
    ...
    self::assertSame(self::REACTING_MEMBER_ID, $result->currentMemberId);
  }
  ...
  public function testInvokeThrowsWhenTheEmojiIsNotAPlausibleGrapheme(): void
  {
    ...
    $reactions = $this->createMock(MessagingReactionRepositoryPort::class);
    $reactions->expects(self::never())->method('add');
    ...
    $handler->__invoke(new AddReactionCommand('user-1', self::MESSAGE_ID, 'not an emoji'));
  }
